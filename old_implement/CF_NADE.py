...
class CF_NADE():
	def __init__(self, sess, flags):
		...
		self.input_mask = tf.placeholder('int32', [None, flags.movie_dim])
		self.output_mask = tf.placeholder('int32', [None, flags.movie_dim])
	...
	def buildGraph(self, flags):
		...
		self.X = tf.one_hot(self.X - 1, axis=-1, depth=flags.num_classes)
		self.X = tf.cumsum(self.X, axis=-1, reverse=True)

		self.input_mask = tf.expand_dims(self.input_mask, 2)
		self.input_mask = tf.tile(self.input_mask, [1,1,flags.num_classes])
		self.output_mask = tf.expand_dims(self.output_mask, 2)
		self.output_mask = tf.tile(self.output_mask, [1,1,flags.num_classes])
		#dim(self.H) = batch_size * hidden_dim
		self.H = tf.tanh(tf.add(self.bias['b_hidden'],tf.tensordot(tf.multiply(self.X, self.input_mask), self.weights['W'], axes=[[1,2],[0,1]])))
		#dim(self.output_layer) = batch_size * movie_dim * num_classes
		self.output_layer = tf.add(self.bias['b_output'], tf.multiply(tf.tensordot(self.H, self.weights['Output_W'], axes=[[1],[0]]), self.output_mask))
		...
		self.pred_scores = tf.tensordot(self.scores_prob, [1,2,3,4,5], axes=[[2],[0]])

		self.true_scores = self.X * self.output_mask
		#dim(self.true_scores_onehot) = batch * movie * num_classes
		self.true_scores_onehot = tf.one_hot(self.true_scores - 1, axis=-1, depth=flags.num_classes)
