...
class CF_NADE():
	def __init__(self, sess, flags):
		...
		'W2' : tf.Variable(tf.random_normal([flags.hidden_dim, 500])),
	    ...
	    'b_hidden2':tf.Variable(tf.random_normal([500])),
	...
	def buildGraph(self, flags):
		...
		self.H = tf.sigmoid(tf.add(self.bias['b_hidden'],tf.tensordot(tf.multiply(self.int_X, self.input_mask_2), self.weights['W'], axes=[[1,2],[0,1]])))

		self.H2 = tf.sigmoid(tf.add(self.bias['b_hidden2'], tf.matmul(self.H, self.weights['W2'])))
		#dim(self.output_layer) = batch_size * movie_dim * num_classes
		self.output_layer = tf.add(self.bias['b_output'], tf.multiply(tf.tensordot(self.H2, self.weights['Output_W'], axes=[[1],[0]]), self.output_mask_2))
